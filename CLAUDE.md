...
- ✅ **Extraction Profile Schemas** (commit 8e2155f) — Context-aware entity extraction configuration system
  ...
  - **Purpose:** Enables targeted entity extraction based on context and question focus, closes GAP-ANALYSIS P1 item
  ...
  - **Integration:** Will be used by Extraction Engine service (P0 priority per GAP-ANALYSIS)
...
- **Highlights API, Facet Decomposition, ExtractionProfile Schema** (commit 3fd956b) — Three major additions supporting context-aware exploration:
  - **Highlights API** (`/highlights`) — Complete CRUD system for reader highlights with CFI-based location tracking
    ...
    - Schemas: `ies/backend/src/ies_backend/schemas/highlight.py` — Highlight, HighlightCreate, HighlightUpdate, HighlightBatchSync
    - Service: `ies/backend/src/ies_backend/services/highlight_service.py` — In-memory MVP storage with book indexing
    - Frontend: `ies/reader/src/services/highlightApi.ts` — TypeScript client with network-aware API URLs
    - Reader Integration: `ies/reader/src/components/Reader.tsx` — Text selection triggers highlight creation with CFI preservation
    - Features: Create/update/delete highlights, filter by book or context, batch sync, entity extraction ready, SiYuan sync hooks
    - Data Model: book_id (calibre), text, cfi, note, color (5 options), context_id, chapter, entity_refs, processed flag
  - **Facet Decomposition System** — AI-powered concept breakdown for deeper exploration
    - Graph API: `GET /graph/entity/{name}/facets` — Returns AI-generated facets for any entity (Anthropic Claude integration)
    - Schemas: `ies/backend/src/ies_backend/schemas/graph.py` — Facet, FacetEntity, EntityFacetsResponse
    - Service: `ies/backend/src/ies_backend/services/graph_service.py` — `get_entity_facets()` with Claude Sonnet 4 integration
    - Facet Types: components (parts), mechanisms (how it works), applications (use cases), contrasts (distinctions), questions (open questions)
    - Auto-generation: If entity has no pre-defined facets, AI analyzes entity and generates 3-5 relevant facets
    - Purpose: Enables "drill into concept" exploration flow in Flow Mode — entity → facet → sub-entities
  - **ExtractionProfile Schema** — Context-aware extraction configuration
    - Schema: `ies/backend/src/ies_backend/schemas/extraction.py` (133 lines) — Complete type system for targeted extraction
    - Types: ExtractionProfile, QuestionExtractionProfile, ExtractionResult, ExtractionRunRequest/Response
    - Profile Fields: context_id, core_concepts, synonyms, relation_types, domain_filters, question_overrides
    - Question Overrides: Per-question extraction settings (focus_concepts, relation_types, depth 1-3)
    - Purpose: Guides extraction engine to focus on concepts relevant to active context/question
    - Integration: Ready for Extraction Engine implementation (redux spec compliance)
  - **Gap Analysis Updated** — `docs/GAP-ANALYSIS-2025-12-09.md` now reflects P0/P1 items completed
    - ExtractionProfile: Status changed from "Not implemented" to "Schema complete"
    - Highlights: New P0 item completed
    - Facets: New exploration capability added
